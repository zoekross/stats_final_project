...
ggplot(data = bob_ross) +
    geom_bar(aes(x = tree), fill = "lightblue", color= "white", bins = 10)
```
...
```{r echo=FALSE}
ggplot(data = bob_ross) +
    geom_bar(aes(x = flowers), fill = "lightblue", color= "white", bins = 10)
```

The vast majority of Bob Ross's painting do NOT have flowers in them.
...
Only about 20% of the painting that have flowers have have trees.

```{r echo=FALSE}
ggplot(data = bob_ross) +
    geom_bar(aes(x = clouds), fill = "lightblue", color= "white", bins = 10)
```
...
About 75% of the painting that have clouds have trees.


```{r echo=FALSE}
ggplot(data = bob_ross) +
    geom_bar(aes(x = rocks), fill = "lightblue", color= "white", bins = 10)
```
...
On average, the presence of flowers in a painting increases the odds of having a tree in the painting .596. This makes sense and is in accordance with what we expected from the graph above. We chose to fit this model because our graph of tree and clouds didn't have a particularly good relationship and we wanted to make sure that was the case.
...
The p value is greater than .05 there we can accept the null hypothesis and determine that there is no relationship between flowers and tree.
...
On average, the presence of rocks in a painting increases the odds of having a tree in the painting by 0.293. This makes sense and is in accordance with what we expected from the graph above. Again, we chose to fit this model because our graph of tree and rocks had an good relationship that we can investigate further.
...
We chose to fit this model because we wanted to see what is the effect of combining the beach and clouds variables. By adding the additional independent variable, the odds for the clouds coefficient increase to 0.233, on average. The presence of rocks in a painting increases the odds of a tree being present in the painting by 0.33, on average.  
...
The p-value is less the 0.05 therefore we can reject the null hypothesis, meaning there is a relationship between trees and rocks, accounted for clouds. 
